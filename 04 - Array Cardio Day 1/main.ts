...
const category: HTMLDivElement | null = document.querySelector('.mw-category');
const links: NodeListOf<HTMLAnchorElement> | undefined = category?.querySelectorAll('a');
const de = [...links || []].map(({textContent}) => textContent).filter((item) => item?.includes('de'));
console.log(de);
...
const countData = data.reduce((result: IMap, item) => {
